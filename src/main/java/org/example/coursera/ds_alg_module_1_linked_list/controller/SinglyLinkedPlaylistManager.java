package org.example.coursera.ds_alg_module_1_linked_list.controller;


import org.example.coursera.ds_alg_module_1_linked_list.model.Song;
...
public class SinglyLinkedPlaylistManager {
    ...
    public void addFavoriteSong(Song newSong) {
        ...
        if (isDuplicate(newSong)) {
           System.out.println("This song is duplicated");
           return;
        }
        Node song = new Node(newSong) ;
        if (isEmpty()) {
            this.head = song;
            current = head;
        } else {
    ...
    public void removeFavoriteSong(int index) {
        ...
        if (isEmpty()) {
            System.out.println("Playlist is empyty");
            return;
        }
        if (index == 1) {

            System.out.printf("\n %s is removed successfully", head.song.getTitle());
            head = head.next;
            return;
        }
        int i = 1;
        ...
        System.out.println("Invalid index");}
            ...
            System.out.printf("\n %s is removed successfully", current.song.getTitle());
    ...
    public void displayFavoriteSongs() {
        ...
        int index = 1;
        while(current != null) {
            System.out.printf("%d:  %s", index, current.song.getTitle());
            current = current.next;
            index++;
        }
    ...
    public void playNextFavoriteSong() {
        if(current == null || current.next == null) {
            System.out.println("No next song in the playlists");
            return;
        }
        current = current.next;
        System.out.printf("next song: %s", current.song.toString());
